...
| G-02 | Cùng một chức năng dùng cùng một nhãn ở mọi màn hình (không lúc "Export" lúc "Xuất file") |  | N/A | N/A | **C1:** ⏳ Đo được — nút trên 3 màn hình admin: Users [Export, Add User] · Categories [Add Category] · Campuses [Add Campus]. Bạn xét mẫu đặt tên "Add X" có nhất quán không<br>**C2:** Tính nhất quán nhãn đo ở cấp ứng dụng trên C1<br>**C3:** Tính nhất quán nhãn đo ở cấp ứng dụng trên C1 |
| G-03 | Trong bảng dữ liệu: text canh trái, số canh phải, trạng thái canh giữa — nhất quán ở mọi bảng |  | N/A | N/A | **C1:** ⏳ Đo được — canh lề: USER=start, sáu cột còn lại=center. Bạn xét: MEMBER CODE là số nhưng canh giữa, có nhất quán với quy ước không<br>**C2:** Dialog không có bảng dữ liệu<br>**C3:** Dialog không có bảng dữ liệu |
...
| G-05 | Màu dùng đúng ngữ nghĩa: màu chính cho hành động chính, **đỏ chỉ dành cho lỗi / hành động phá huỷ** |  | Passed | Passed | **C1:** ⏳ Đo được — phần tử màu đỏ duy nhất là nút "Delete user". Bạn xét xem có chỗ nào dùng đỏ sai ngữ cảnh không<br>**C2:** Nhìn ảnh: trong dialog màu đỏ chỉ dùng cho **chữ thông báo lỗi**, không dùng cho hành động thường<br>**C3:** Nhìn ảnh: trong dialog màu đỏ chỉ dùng cho **chữ thông báo lỗi**, không dùng cho hành động thường |
...
| G-08 | Trạng thái rỗng (empty state) có thông điệp giải thích **và** gợi ý hành động tiếp theo, không phải vùng trắng trơn | Failed | N/A | N/A | **C1:** Empty state hiện đúng thông điệp giải thích: *"No users found matching your filters."* nhưng **không có gợi ý hành động tiếp theo** — không nút "Xoá bộ lọc", không link nào. Mục này đòi cả hai. *(Ranh giới — câu chữ có ám chỉ "filters" nên bạn có thể nâng lên Passed nếu thấy đủ)*<br>**C2:** Dialog không có trạng thái rỗng<br>**C3:** Dialog không có trạng thái rỗng |
...
| F-06 | Giá trị mặc định của mỗi trường hợp lý (vd ngày bắt đầu = hôm nay), không rỗng hay vô nghĩa | N/A |  |  | **C1:** Ô tìm kiếm không cần giá trị mặc định<br>**C2:** ⏳ Giá trị mặc định C2: điền sẵn dữ liệu user đang sửa (Nguyen / Trang / email), Active=true. Bạn xét có hợp lý không<br>**C3:** ⏳ Giá trị mặc định C3: mọi ô rỗng, riêng checkbox Active=true. Bạn xét mặc định bật Active có hợp lý không |
...
| N-16 | Cột bảng sắp xếp được phải có **chỉ báo hướng sắp xếp** (mũi tên tăng/giảm) và giữ nguyên tiêu chí sắp xếp khi chuyển trang |  | N/A | N/A | **C1:** ⏳ Không bấm được tiêu đề cột bằng script, không có `aria-sort`. Bạn tự bấm để xem có sắp xếp và chỉ báo hướng không<br>**C2:** Dialog không có cột sắp xếp<br>**C3:** Dialog không có cột sắp xếp |
